...
import numpy as np
from graph_tool.centrality import trust_transitivity
...
class GTTransitivity(WSDAlgorithmInterface):
  """!
  ...
  def __init__(self, str_name = 'GTTransitivity'):
    """!
    @param str_name - default value same as the class name "GTPersonalizedPR"
    """
    super(GTTransitivity, self).__init__(str_name)
  ...
  def run(self, wsd_context, graph, options, resources):
    ...
    print "Number vertices: " + str(gtGraph.num_vertices()) + "\n"
    vertex_trust = trust_transitivity(gtGraph)
				   #trust_map = propMap,
                                   #pers = pers_v,                       
                                   #max_iter = 2 * options.max_iter(),
                                   #damping = options.damping_factor(),
    ...
    for (lemma, pos_str) in lemma_on_only_synset_node_dict.iterkeys():
      wsd_rank.set_ranking_for_lemma(lemma, pos_str, vertex_trust)
