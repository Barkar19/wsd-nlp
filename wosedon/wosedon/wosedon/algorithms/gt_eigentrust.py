...
import numpy as np
from graph_tool.centrality import eigentrust
...
class GTEigentrust(WSDAlgorithmInterface):
  """!
  ...
  def __init__(self, str_name = 'GTEigentrust'):
    """!
    @param str_name - default value same as the class name "GTPersonalizedPR"
    """
    super(GTEigentrust, self).__init__(str_name)
  ...
  def run(self, wsd_context, graph, options, resources):
    ...
    print "Number vertices: " + str(gtGraph.num_vertices()) + "\n"
    vertex_trust = eigentrust(gtGraph)
				   #trust_map = propMap)
                                   #pers = pers_v,                       
                                   #max_iter = 2 * options.max_iter(),
                                   #damping = options.damping_factor(),
