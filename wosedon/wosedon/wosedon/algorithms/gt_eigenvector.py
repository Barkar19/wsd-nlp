...
class GTEigenvector(WSDAlgorithmInterface):
  """!
  ...
  def __init__(self, str_name = 'GTEigenvector'):
    """!
    @param str_name - default value same as the class name "GTPersonalizedPR"
    """
    super(GTEigenvector, self).__init__(str_name)
  ...
  def run(self, wsd_context, graph, options, resources):
    ...
    print "Number vertices: " + str(gtGraph.num_vertices()) + "\n"
    (_, vertex_eigenvector) = eigenvector(gtGraph,
                                   #pers = pers_v,                       
                                   max_iter = 2 * options.max_iter(),
                                   #damping = options.damping_factor(),
                                   #ret_iter = True,
                                   weight = gtGraph.ep["weight"])
    print "vertex_eigenvector size: " + str(vertex_eigenvector.get_array().size)

    vertex_eigenvector = graph.ungraph_tool(vertex_eigenvector)
    #edge_betweenness = graph.ungraph_tool(edge_betweenness)
    print "vertex_eigenvector after ungraph: " + str(type(vertex_eigenvector)) + " " + str(len(vertex_eigenvector)) 



    for (lemma, pos_str) in lemma_on_only_synset_node_dict.iterkeys():
      wsd_rank.set_ranking_for_lemma(lemma, pos_str, vertex_eigenvector)
