...
def show_window_relations(np_adjp_phrase, out_dirs):
	...
	print ''
	print 20 * '  - '
	print 'Podaj po kropce kolejne elementy relacji np. 1~>2.3'
	print 'Kolejne relacje oddzielaj od siebie srednikiem, np. 1<~2.3; 2<~3 '
	relations = raw_input(':')
	...
	for rel in rel_spl:
		...
		if rel.find('~>') != -1:
			...
			r1 = rel.split('<~')[1].split('.')
			r2 = rel.split('<~')[0]
			ret_items = [r2] + r1
			# r2 = rel.split('<~')[0].split('.')
			# ret_items = r2 + r1
		else:
			ret_items = [int(r) for r in rel.split('.')]
		ret_items = [int(r) for r in ret_items]
		ret_rel.append(ret_items)
	return ret_rel
...
def show_window_additional_relations(np_adjp_phrase, phr_str, out_dirs):
	...
	'''
	relations = []
	other_relations = raw_input('[?] Czy znajduja sie inne, nieznalezione relacje skladniowe (t/n)?')
	if other_relations.upper() == 'T':
		relations = show_window_relations(np_adjp_phrase, out_dirs)

	for rel_items in relations:
		print rel_items
		srel = syntx_rel.syntax_relation(np_adjp_phrase, rel_items)
		user_decision = ev.SingleResult(srel)
		user_decision.correct_pair_triple = False
		user_decision.incorrect_auto_relation = False
		user_decision.auto_semantic_relation = None
		user_decision.manual_semantic_relation = False
...
def show_window(rule_point_str, syntax_relation, semantic_relation, wcc_op_cfg_file, phr_str, eval_count, only_gen_rel):
	...
	corr_pair_triple = raw_input('[?] Czy wygenerowana relacja skladniowa jest poprawna (t/n)?')
